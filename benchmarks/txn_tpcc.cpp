...
void TxnManTPCC::init(uint64_t thd_id, WLSchema * h_wl) {
...
RC TxnManTPCC::run_txn_post_wait() {
  ...
  switch(tpcc_query->txn_type) {
    case TPCC_PAYMENT:
      ...
      // WH
        ...
        rc2 = get_lock(row,g_update_wh? WR:RD);
        if(rc2 != RCOK)
          rc = rc2;
...
void TxnManTPCC::next_tpcc_state() {
  //QryTPCC* tpcc_query = (QryTPCC*) query;
...
bool TxnManTPCC::is_local_item(uint64_t idx) {
  QryTPCC* tpcc_query = (QryTPCC*) query;
...
void TxnManTPCC::copy_remote_items(QueryMsgTPCC * message) {
  QryTPCC* tpcc_query = (QryTPCC*) query;
  message->items.init(tpcc_query->items.size());
  if(tpcc_query->txn_type == TPCC_PAYMENT)
    return;
  ...
  while(next_item_id < tpcc_query->items.size() && !is_local_item(next_item_id) && GET_NODE_ID(wh_to_part(tpcc_query->items[next_item_id]->ol_supply_w_id)) == dest_node_id) {
    Item_no * req = (Item_no*) alloc_memory.alloc(sizeof(Item_no));
    req->copy(tpcc_query->items[next_item_id++]);
    message->items.add(req);
  }
}


RC TxnManTPCC::run_txn_state() {
    QryTPCC* tpcc_query = (QryTPCC*) query;
    uint64_t w_id = tpcc_query->w_id;
    uint64_t d_id = tpcc_query->d_id;
    uint64_t c_id = tpcc_query->c_id;
...
inline RC TxnManTPCC::run_payment_0(uint64_t w_id, uint64_t d_id, uint64_t d_w_id, double h_amount, RowData *& r_wh_local) {
...
/*====================================================+
  ...
  RC rc;
	...
	item = index_read(index, key, wh_to_part(w_id));
	assert(item != NULL);
	RowData * r_wh = ((RowData *)item->location);
	if (g_update_wh)
...
inline RC TxnManTPCC::run_payment_1(uint64_t w_id, uint64_t d_id, uint64_t d_w_id, double h_amount, RowData * r_wh_local) {
...
/*====================================================+
	...
	double w_ytd;
	r_wh_local->get_row_value(W_YTD, w_ytd);
	if (g_update_wh) {
...
inline RC TxnManTPCC::run_payment_2(uint64_t w_id, uint64_t d_id, uint64_t d_w_id, double h_amount, RowData *& r_dist_local) {
	/*=====================================================+
	...
	item = index_read(_wl->i_district, key, wh_to_part(w_id));
	assert(item != NULL);
	RowData * r_dist = ((RowData *)item->location);
	RC rc = get_row(r_dist, WR, r_dist_local);
  return rc;

}

inline RC TxnManTPCC::run_payment_3(uint64_t w_id, uint64_t d_id, uint64_t d_w_id, double h_amount, RowData * r_dist_local) {
  assert(r_dist_local != NULL);
	...
	+=====================================================*/
	double d_ytd;
	r_dist_local->get_row_value(D_YTD, d_ytd);
	r_dist_local->set_value(D_YTD, d_ytd + h_amount);

	return RCOK;
}

inline RC TxnManTPCC::run_payment_4(uint64_t w_id, uint64_t d_id,uint64_t c_id,uint64_t c_w_id, uint64_t c_d_id, char * c_last, double h_amount, bool by_last_name, RowData *& r_cust_local) { 
	/*====================================================================+
...
inline RC TxnManTPCC::run_payment_5(uint64_t w_id, uint64_t d_id,uint64_t c_id,uint64_t c_w_id, uint64_t c_d_id, char * c_last, double h_amount, bool by_last_name, RowData * r_cust_local) { 
  assert(r_cust_local != NULL);
	double c_balance;
	double c_ytd_payment;
	double c_payment_cnt;

	r_cust_local->get_row_value(C_BALANCE, c_balance);
	r_cust_local->set_value(C_BALANCE, c_balance - h_amount);
	r_cust_local->get_row_value(C_YTD_PAYMENT, c_ytd_payment);
	r_cust_local->set_value(C_YTD_PAYMENT, c_ytd_payment + h_amount);
	r_cust_local->get_row_value(C_PAYMENT_CNT, c_payment_cnt);
	r_cust_local->set_value(C_PAYMENT_CNT, c_payment_cnt + 1);

	//char * c_credit = r_cust_local->get_row_value(C_CREDIT);
...
// new_order 0
inline RC TxnManTPCC::new_order_0(uint64_t w_id, uint64_t d_id, uint64_t c_id, bool remote, uint64_t  ol_cnt,uint64_t  o_entry_d, uint64_t * o_id, RowData *& r_wh_local) {
	uint64_t key;
	itemidData * item;
	/*=======================================================================+
...
inline RC TxnManTPCC::new_order_1(uint64_t w_id, uint64_t d_id, uint64_t c_id, bool remote, uint64_t  ol_cnt,uint64_t  o_entry_d, uint64_t * o_id, RowData * r_wh_local) {
  assert(r_wh_local != NULL);
	double w_tax;
	r_wh_local->get_row_value(W_TAX, w_tax); 
  return RCOK;
}

inline RC TxnManTPCC::new_order_2(uint64_t w_id, uint64_t d_id, uint64_t c_id, bool remote, uint64_t  ol_cnt,uint64_t  o_entry_d, uint64_t * o_id, RowData *& r_cust_local) {
	uint64_t key;
	itemidData * item;
	key = custKey(c_id, d_id, w_id);
	INDEX * index = _wl->i_customer_id;
	item = index_read(index, key, wh_to_part(w_id));
	assert(item != NULL);
	RowData * r_cust = (RowData *) item->location;
  RC rc = get_row(r_cust, RD, r_cust_local);
  return rc;
}

inline RC TxnManTPCC::new_order_3(uint64_t w_id, uint64_t d_id, uint64_t c_id, bool remote, uint64_t  ol_cnt,uint64_t  o_entry_d, uint64_t * o_id, RowData * r_cust_local) {
  assert(r_cust_local != NULL);
	uint64_t c_discount;
	//char * c_last;
	//char * c_credit;
	r_cust_local->get_row_value(C_DISCOUNT, c_discount);
	//c_last = r_cust_local->get_row_value(C_LAST);
	//c_credit = r_cust_local->get_row_value(C_CREDIT);
  return RCOK;
}
 	
inline RC TxnManTPCC::new_order_4(uint64_t w_id, uint64_t d_id, uint64_t c_id, bool remote, uint64_t  ol_cnt,uint64_t  o_entry_d, uint64_t * o_id, RowData *& r_dist_local) {
	uint64_t key;
	itemidData * item;
	/*==================================================+
...
inline RC TxnManTPCC::new_order_5(uint64_t w_id, uint64_t d_id, uint64_t c_id, bool remote, uint64_t  ol_cnt,uint64_t  o_entry_d, uint64_t * o_id, RowData * r_dist_local) {
  assert(r_dist_local != NULL);
	//double d_tax;
	//int64_t o_id;
	//d_tax = *(double *) r_dist_local->get_row_value(D_TAX);
	*o_id = *(int64_t *) r_dist_local->get_row_value(D_NEXT_O_ID);
	(*o_id) ++;
...
inline RC TxnManTPCC::new_order_6(uint64_t ol_i_id, RowData *& r_item_local) {
		uint64_t key;
		itemidData * item;
		/*===========================================+
		...
		item = index_read(_wl->i_item, key, 0);
		assert(item != NULL);
		RowData * r_item = ((RowData *)item->location);

    RC rc = get_row(r_item, RD, r_item_local);
    return rc;
}

inline RC TxnManTPCC::new_order_7(uint64_t ol_i_id, RowData * r_item_local) {
  assert(r_item_local != NULL);
		int64_t i_price;
		//char * i_name;
		//char * i_data;
		
		r_item_local->get_row_value(I_PRICE, i_price);
		//i_name = r_item_local->get_row_value(I_NAME);
		//i_data = r_item_local->get_row_value(I_DATA);
...
// new_order 2
inline RC TxnManTPCC::new_order_8(uint64_t w_id,uint64_t  d_id,bool remote, uint64_t ol_i_id, uint64_t ol_supply_w_id, uint64_t ol_quantity,uint64_t  ol_number,uint64_t  o_id, RowData *& r_stock_local) {
		uint64_t key;
		itemidData * item;
		...
		assert(item != NULL);
		RowData * r_stock = ((RowData *)item->location);
    RC rc = get_row(r_stock, WR, r_stock_local);
    return rc;
}
		
inline RC TxnManTPCC::new_order_9(uint64_t w_id,uint64_t  d_id,bool remote, uint64_t ol_i_id, uint64_t ol_supply_w_id, uint64_t ol_quantity,uint64_t  ol_number, uint64_t ol_amount, uint64_t  o_id, RowData * r_stock_local) {
  assert(r_stock_local != NULL);
		// XXX s_dist_xx are not retrieved.
		UInt64 s_quantity;
		int64_t s_remote_cnt;
		s_quantity = *(int64_t *)r_stock_local->get_row_value(S_QUANTITY);
#if !SMALL_TPCC
		int64_t s_ytd;
		int64_t s_order_cnt;
		char * s_data __attribute__ ((unused));
		r_stock_local->get_row_value(S_YTD, s_ytd);
		r_stock_local->set_value(S_YTD, s_ytd + ol_quantity);
    // In Coordination Avoidance, this record must be protected!
		r_stock_local->get_row_value(S_ORDER_CNT, s_order_cnt);
		r_stock_local->set_value(S_ORDER_CNT, s_order_cnt + 1);
		s_data = r_stock_local->get_row_value(S_DATA);
#endif
		if (remote) {
			s_remote_cnt = *(int64_t*)r_stock_local->get_row_value(S_REMOTE_CNT);
			s_remote_cnt ++;
  ...
  while(!calvin_exec_phase_done() && rc == RCOK) {
    ...
    switch(this->phase) {
      case CALVIN_RW_ANALYSIS:
        ...
        DEBUG("(%ld,%ld) expects %d responses; %ld participants, %ld active\n",txn->txn_id,txn->batch_id,calvin_expected_rsp_cnt,query->related_nodes.size(),query->nodes_active.size());
  ...
  //bool c_w_loc = GET_NODE_ID(part_id_c_w) == g_node_id;
	...
	switch (tpcc_query->txn_type) {
      ...
      if(w_loc) {
        ...
        rc = new_order_3( w_id, d_id, c_id, remote, ol_cnt, o_entry_d, &tpcc_query->o_id, row); 
        rc = new_order_4( w_id, d_id, c_id, remote, ol_cnt, o_entry_d, &tpcc_query->o_id, row); 
        tpcc_query->o_id = *(int64_t *) row->get_row_value(D_NEXT_O_ID);
        //rc = new_order_5( w_id, d_id, c_id, remote, ol_cnt, o_entry_d, &tpcc_query->o_id, row); 
      }
        for(uint64_t i = 0; i < tpcc_query->ol_cnt; i++) {
