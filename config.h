...
// each transaction only access only 1 virtual partition. But the lock/ts manager and index are not aware of such partitioning. VIRTUAL_PART_CNT describes the request distribution and is only used to generate queries. For HSTORE, VIRTUAL_PART_CNT should be the same as PART_CNT.
...
//    HOT: use ACCESS_PERC of the access go to DATA_PERC of the data
...
// % of transactions that access multiple parts_assign
#define MPR 1.0
#define MPIR 0.01
