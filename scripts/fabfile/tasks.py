...
def copy_schema():
    if env.dry_run:
        return
    schemas = ["benchmarks/schema_f_TPCC.txt","benchmarks/YCSB_schema.txt","benchmarks/PPS_schema.txt"]
...
#                    message = "Not enough nodes to run experiment!\n"
#                    message += "\tRequired nodes: {}, ".format(ntotal)
#                    message += "Actual nodes: {}".format(len(env.hosts))
#                    with color():
#                        puts(message,show_prefix=True)
...
#                        outfiles[host] = "{}_{}.out".format(nid[0],output_f) 
                    ...
                    if cfgs["WORKLOAD"] == "TPCC":
                        schema = "benchmarks/schema_f_TPCC.txt"
                    elif cfgs["WORKLOAD"] == "YCSB":
                        schema = "benchmarks/schema_YCSB.txt"
