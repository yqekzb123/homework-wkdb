...
stat_map = OrderedDict([
  ...
  ('task_queue_cnt', []),
  ('task_queue_enq_cnt', []),
  ('task_queue_wait_avg_time', []),
  ('task_queue_mtx_wait_time', []),
  ('task_queue_mtx_wait_avg', []),
  ('task_queue_new_cnt', []),
  ('task_queue_new_wait_time', []),
  ('task_queue_new_wait_avg_time', []),
  ('task_queue_old_cnt', []),
  ('task_queue_old_wait_time', []),
  ('task_queue_old_wait_avg_time', []),
  ('task_queue_enqueue_time', []),
  ('task_queue_dequeue_time', []),
  ...
  ('occ_template_validate_time', []),
  ('occ_template_validate_avg', []),
  ('occ_template_cs_wait_time', []),
  ('occ_template_cs_wait_avg', []),
...
#    pp.pprint(summary['txn_cnt'])
    if "task_queue_enq_cnt" in summary and "message_queue_cnt" in summary:
        wq_diff = summary["task_queue_enq_cnt"][-1] - summary["task_queue_cnt"][-1]
        mq_diff = summary["message_queue_cnt"][-1] - summary["msg_queue_cnt"][-1]
        if  wq_diff > 5000:
            print("Work Queue: {}".format(wq_diff))
        if  mq_diff > 5000:
            print("Msg Queue: {}".format(mq_diff))
    if "seq_cnt_batch" in summary and "sched_epoch_cnt" in summary:
        if summary["sched_epoch_cnt"][-1] > summary["seq_cnt_batch"][-1] :
            print("Sched epcoh {} > seq epoch {}".format(summary["sched_epoch_cnt"][-1],summary["seq_cnt_batch"][-1]))
