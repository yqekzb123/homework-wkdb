...
#bbox = [0.7,0.9]
    ...
    # FIXME (Dana): OCCTEMPLATE --> OCC quick fix
    print tpt.keys()
    if "OCCTEMPLATE" in tpt.keys():
        occ_tput = tpt["OCCTEMPLATE"]
        del tpt["OCCTEMPLATE"]
        tpt["OCC"] = occ_tput
...
def lat(xval,vval,summary,
    ...
    for stat in stat_types:
        ...
        _title = 'Txn Latency-{}% {}'.format(stat,title)
        lat = {}
...
#            my_cfg[my_cfg_fmt.index(vname)] = v
                ...
                n_alg =  my_cfg[my_cfg_fmt.index("ALGO")]
...
def lat_node_tbls(exp,nodes,msg_bytes,ts,
    ...
    for i,b in enumerate(msg_bytes):
        for s in stats:
            _title="{} % Latencies (ms), Msg Size of {} bytes".format(s,b)
...
def lat_tbl(totals,msg_bytes,ts,
    ...
    _title="Latencies Stats(ms), Msg Size of {} bytes".format("all")
...
def tportlat_mpr(mpr,nodes,algos,max_txn,summary):
    ...
    draw_line(name,tport_lat,mpr,ylab='Latency (s)',xlab='Multi-Partition Rate',title='Average Msg Latency',bbox=[0.5,0.95],ltitle=vname) 
...
#FIXME
    ...
    if "OCCTEMPLATE" in _xval:
        _xval[_xval.index("OCCTEMPLATE")] = "OCC"
...
#            time_abort[i] = avg(summary[cfgs]['lat_l_loc_abort_time']) / txn_cnt[i]
            ...
            time_work_q[i] = (sum(summary[cfgs]['lat_short_task_queue_time']) ) / txn_cnt[i]
...
#            time_work_q[i] = (sum(summary[cfgs]['lat_s_loc_task_queue_time']) + avg(summary[cfgs]['lat_l_rem_task_queue_time']) * avg_parts_touched) / txn_cnt[i]
...
#            time_work_q[i] = (sum(summary[cfgs]['lat_s_loc_task_queue_time']) + avg(summary[cfgs]['lat_l_rem_task_queue_time']) ) / txn_cnt[i]
...
#            total[i] += time_network[i]
            ...
            ltime_work_q[i] = (sum(summary[cfgs]['lat_l_loc_task_queue_time']) + sum(summary[cfgs]['lat_l_rem_task_queue_time'])+ sum(summary[cfgs]['lat_s_rem_task_queue_time'])) / txn_cnt[i]
