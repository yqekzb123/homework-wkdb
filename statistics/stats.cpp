...
void Stats_thd::init(uint64_t thd_id) {
  ...
  part_cnt = (uint64_t*) alloc_memory.align_alloc(sizeof(uint64_t)*g_cnt_part);
  DEBUG_M("Stats_thd::init part_acc alloc\n");
  part_acc = (uint64_t*) alloc_memory.align_alloc(sizeof(uint64_t)*g_cnt_part);
  DEBUG_M("Stats_thd::init worker_process_cnt_by_type alloc\n");
  worker_process_cnt_by_type= (uint64_t *) alloc_memory.align_alloc(sizeof(uint64_t) * WKNO_MSG);
  DEBUG_M("Stats_thd::init worker_process_time_by_type alloc\n");
  worker_process_time_by_type= (double *) alloc_memory.align_alloc(sizeof(double) * WKNO_MSG);
  DEBUG_M("Stats_thd::init mtx alloc\n");
  mtx= (double *) alloc_memory.align_alloc(sizeof(double) * 40);

	//all_lat.init(g_per_part_max_txn,ArrIncr);

	client_client_latency.init(g_per_part_max_txn,ArrIncr);
	last_start_commit_latency.init(g_per_part_max_txn,ArrIncr);
	first_start_commit_latency.init(g_per_part_max_txn,ArrIncr);
	start_abort_commit_latency.init(g_per_part_max_txn,ArrIncr);
...
void Stats_thd::clear() {
  ...
  task_queue_cnt=0;
  task_queue_enq_cnt=0;
  task_queue_mtx_wait_time=0;
  task_queue_new_cnt=0;
  task_queue_new_wait_time=0;
  task_queue_old_cnt=0;
  task_queue_old_wait_time=0;
  task_queue_enqueue_time=0;
  task_queue_dequeue_time=0;
  task_queue_conflict_cnt=0;
  ...
  // OCCTEMPLATE
  occ_template_validate_cnt=0;
  occ_template_validate_time=0;
  occ_template_cs_wait_time=0;
  occ_template_case1_cnt=0;
  occ_template_case2_cnt=0;
  occ_template_case3_cnt=0;
  occ_template_case4_cnt=0;
  occ_template_case5_cnt=0;
  occ_template_range=0;
  occ_template_commit_cnt=0;
...
void Stats_thd::print(FILE * outf, bool prog) {
  ...
  double task_queue_wait_avg_time = 0;
  double task_queue_mtx_wait_avg = 0;
  double task_queue_new_wait_avg_time = 0;
  double task_queue_old_wait_avg_time = 0;
  if(task_queue_cnt > 0) {
    task_queue_wait_avg_time = task_queue_wait_time / task_queue_cnt;
    task_queue_mtx_wait_avg = task_queue_mtx_wait_time / task_queue_cnt;
  }
  if(task_queue_new_cnt > 0)
    task_queue_new_wait_avg_time = task_queue_new_wait_time / task_queue_new_cnt;
  if(task_queue_old_cnt > 0)
    task_queue_old_wait_avg_time = task_queue_old_wait_time / task_queue_old_cnt;
  // Work queue
  fprintf(outf,
  ",task_queue_wait_time=%f"
  ",task_queue_cnt=%ld"
  ",task_queue_enq_cnt=%ld"
  ",task_queue_wait_avg_time=%f"
  ",task_queue_mtx_wait_time=%f"
  ",task_queue_mtx_wait_avg=%f"
  ",task_queue_new_cnt=%ld"
  ",task_queue_new_wait_time=%f"
  ",task_queue_new_wait_avg_time=%f"
  ",task_queue_old_cnt=%ld"
  ",task_queue_old_wait_time=%f"
  ",task_queue_old_wait_avg_time=%f"
  ",task_queue_enqueue_time=%f"
  ",task_queue_dequeue_time=%f"
  ",task_queue_conflict_cnt=%ld"
  ,task_queue_wait_time / BILLION
  ,task_queue_cnt
  ,task_queue_enq_cnt
  ,task_queue_wait_avg_time / BILLION
  ,task_queue_mtx_wait_time / BILLION
  ,task_queue_mtx_wait_avg / BILLION
  ,task_queue_new_cnt
  ,task_queue_new_wait_time / BILLION
  ,task_queue_new_wait_avg_time / BILLION
  ,task_queue_old_cnt
  ,task_queue_old_wait_time / BILLION
  ,task_queue_old_wait_avg_time / BILLION
  ,task_queue_enqueue_time / BILLION
  ,task_queue_dequeue_time / BILLION
  ,task_queue_conflict_cnt
  );
  ...
  //OCCTEMPLATE
  double occ_template_range_avg = 0;
  double occ_template_validate_avg = 0;
  double occ_template_cs_wait_avg = 0;
  uint64_t occ_template_commit_avg = 0;
  if(occ_template_commit_cnt > 0)
    occ_template_range_avg = occ_template_range / occ_template_commit_cnt;
  if(occ_template_validate_cnt > 0) {
    occ_template_validate_avg = occ_template_validate_time / occ_template_validate_cnt;
    occ_template_cs_wait_avg = occ_template_cs_wait_time / occ_template_validate_cnt;
    occ_template_commit_avg = occ_template_commit_cnt / occ_template_validate_cnt;
  }
  fprintf(outf,
  ",occ_template_validate_cnt=%ld"
  ",occ_template_validate_time=%f"
  ",occ_template_validate_avg=%f"
  ",occ_template_cs_wait_time=%f"
  ",occ_template_cs_wait_avg=%f"
  ",occ_template_case1_cnt=%ld"
  ",occ_template_case2_cnt=%ld"
  ",occ_template_case3_cnt=%ld"
  ",occ_template_case4_cnt=%ld"
  ",occ_template_case5_cnt=%ld"
  ",occ_template_range=%f"
  ",occ_template_commit_cnt=%ld"
  ",occ_template_commit_avg=%ld"
  ",occ_template_range_avg=%f"
  ,occ_template_validate_cnt
  ,occ_template_validate_time / BILLION
  ,occ_template_validate_avg / BILLION
  ,occ_template_cs_wait_time / BILLION
  ,occ_template_cs_wait_avg / BILLION
  ,occ_template_case1_cnt
  ,occ_template_case2_cnt
  ,occ_template_case3_cnt
  ,occ_template_case4_cnt
  ,occ_template_case5_cnt
  ,occ_template_range / BILLION
  ,occ_template_commit_cnt
  ,occ_template_commit_avg
  ,occ_template_range_avg
  );
  ...
  ,lat_l_loc_task_queue_time/BILLION
  ...
  ,lat_short_task_queue_time/BILLION
...
void Stats_thd::combine(Stats_thd * stats) {
  ...
  // Work queue
  task_queue_wait_time+=stats->task_queue_wait_time;
  task_queue_cnt+=stats->task_queue_cnt;
  task_queue_enq_cnt+=stats->task_queue_enq_cnt;
  task_queue_mtx_wait_time+=stats->task_queue_mtx_wait_time;
  task_queue_new_cnt+=stats->task_queue_new_cnt;
  task_queue_new_wait_time+=stats->task_queue_new_wait_time;
  task_queue_old_cnt+=stats->task_queue_old_cnt;
  task_queue_old_wait_time+=stats->task_queue_old_wait_time;
  task_queue_enqueue_time+=stats->task_queue_enqueue_time;
  task_queue_dequeue_time+=stats->task_queue_dequeue_time;
  task_queue_conflict_cnt+=stats->task_queue_conflict_cnt;
  ...
  message_queue_cnt+=stats->message_queue_cnt;
  ...
  seq_cnt_batch+=stats->seq_cnt_batch;
  seq_cnt_full_batch+=stats->seq_cnt_full_batch;
  seq_ack_time+=stats->seq_ack_time;
  seq_batch_time+=stats->seq_batch_time;
  seq_proc_cnt+=stats->seq_proc_cnt;
  ...
  // OCCTEMPLATE
  occ_template_validate_cnt+=stats->occ_template_validate_cnt;
  occ_template_validate_time+=stats->occ_template_validate_time;
  occ_template_cs_wait_time+=stats->occ_template_cs_wait_time;
  occ_template_case1_cnt+=stats->occ_template_case1_cnt;
  occ_template_case2_cnt+=stats->occ_template_case2_cnt;
  occ_template_case3_cnt+=stats->occ_template_case3_cnt;
  occ_template_case4_cnt+=stats->occ_template_case4_cnt;
  occ_template_case5_cnt+=stats->occ_template_case5_cnt;
  occ_template_range+=stats->occ_template_range;
  occ_template_commit_cnt+=stats->occ_template_commit_cnt;
  ...
  lat_task_queue_time+=stats->lat_task_queue_time;
  ...
  lat_l_loc_task_queue_time+=stats->lat_l_loc_task_queue_time;
  ...
  lat_short_task_queue_time+=stats->lat_short_task_queue_time;
  ...
  lat_s_loc_task_queue_time+=stats->lat_s_loc_task_queue_time;
  ...
  lat_l_rem_task_queue_time+=stats->lat_l_rem_task_queue_time;
  ...
  lat_s_rem_task_queue_time+=stats->lat_s_rem_task_queue_time;
...
void Stats::init(uint64_t thread_cnt) {
  ...
  for(uint64_t i = 0; i < thread_cnt; i++) {
    _stats[i] = (Stats_thd *) 
      alloc_memory.align_alloc(sizeof(Stats_thd));
    _stats[i]->init(i);
    _stats[i]->clear();
  }
...
uint64_t Stats::get_txn_cnts() {
    if(!STATS_ENABLE || g_node_id >= g_cnt_node)
        return 0;
    uint64_t limit =  g_thd_cnt + g_rem_thd_cnt;
...
void Stats::cpu_util(FILE * outf) {
  ...
  if (now <= lastCPU || timeSample.tms_stime < lastSysCPU ||
      ...
      percent /= (now - lastCPU);
      if(ISSERVER) {
        percent /= (g_thread_total_cnt);//numProcessors;
      } else if(ISCLIENT){
