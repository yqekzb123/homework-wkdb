...
RC IndexBTree::init(uint64_t part_cnt) {
	...
	ARR_PTR(bt_node *, cur_leaf_per_thd, g_thd_cnt);
	ARR_PTR(UInt32, cur_idx_per_thd, g_thd_cnt);
...
RC IndexBTree::init(uint64_t part_cnt, TableSchema * table) {
	this->table = table;
	init(part_cnt);
	return RCOK;
}

bt_node * IndexBTree::find_root(uint64_t part_id) {
	assert (part_id < part_cnt);
	return roots[part_id];
}

bool IndexBTree::index_is_exist(idx_key_t key) {
...
RC IndexBTree::index_next(uint64_t thd_id, itemidData * &item, bool samekey) {
	...
	*cur_idx_per_thd[thd_id] += 1;
	...
	if (leaf == NULL)
		...
		if ( samekey && leaf->keys[ *cur_idx_per_thd[thd_id] ] != cur_key)
			...
			item = (itemidData *) leaf->pointers[ *cur_idx_per_thd[thd_id] ];
	}
	return RCOK;
}

RC IndexBTree::index_read(idx_key_t key, itemidData *& item) {
	assert(false);
	return RCOK;
}

RC 
IndexBTree::index_read(idx_key_t key, 
	itemidData *& item, 
	int part_id) {
	...
	uint64_t thd_id, int64_t part_id) 
{
	...
	for (UInt32 i = 0; i < leaf->num_keys; i++) 
		if (leaf->keys[i] == key) {
			item = (itemidData *)leaf->pointers[i];
			release_node_latch(leaf);
			(*cur_leaf_per_thd[thd_id]) = leaf;
...
RC IndexBTree::index_insert(idx_key_t key, itemidData * item, int part_id) {
	glob_param params;
...
//	M_ASSERT(!index_is_exist(key), "the index does not exist!");
...
//		assert( release_node_latch(leaf) == LATCH_EX );
		for (int i = 0; i < depth; i++)
			release_node_latch(ex_list[i]);
//			assert( release_node_latch(ex_list[i]) == LATCH_EX );
	}
...
RC IndexBTree::make_lf(uint64_t part_id, bt_node *& node) {
...
RC IndexBTree::make_nl(uint64_t part_id, bt_node *& node) {
	RC rc = make_node(part_id, node);
	if (rc != RCOK) return rc;
	node->is_leaf = false;
	return RCOK;
...
RC IndexBTree::make_node(uint64_t part_id, bt_node *& node) {	
//	printf("make_node(). part_id=%lld\n", part_id);
	bt_node * new_node = (bt_node *) alloc_memory.alloc(sizeof(bt_node));
	assert (new_node != NULL);
	new_node->pointers = NULL;
	new_node->keys = (idx_key_t *) alloc_memory.alloc((order - 1) * sizeof(idx_key_t));
	new_node->pointers = (void **) alloc_memory.alloc(order * sizeof(void *));
...
RC IndexBTree::start_new_tree(glob_param params, idx_key_t key, itemidData * item) {
	RC rc;
...
bool IndexBTree::latch_node(bt_node * node, latch_t latch_type) {
...
latch_t IndexBTree::release_node_latch(bt_node * node) {
	if (!ENABLE_LATCH)
		return LATCH_SH;
...
RC IndexBTree::upgrade_node_latch(bt_node * node) {
	if (!ENABLE_LATCH)
		return RCOK;
	bool success = false;
//	if ( g_cc_alg != HSTORE ) 
...
RC IndexBTree::cleanup(bt_node * node, bt_node * last_ex) {
	if (last_ex != NULL) {
		do {
			node = node->parent;
//			assert(release_node_latch(node) == LATCH_EX);
			release_node_latch(node);
		}
		while (node != last_ex);
	}
	return RCOK;
...
RC IndexBTree::find_leaf(glob_param params, idx_key_t key, idx_acc_t access_type, bt_node *& leaf) {
	bt_node * last_ex = NULL;
...
RC IndexBTree::find_leaf(glob_param params, idx_key_t key, idx_acc_t access_type, bt_node *& leaf, bt_node  *& last_ex) 
{
//	RC rc;
	UInt32 i;
	...
	while (!c->is_leaf) {
		...
		if (access_type == INDEX_INSERT) {
			if (child->num_keys == order - 1) {
				if (upgrade_node_latch(c) != RCOK) {
					release_node_latch(c);
					release_node_latch(child);
					cleanup(c, last_ex);
					last_ex = NULL;
					return Abort;
				}
			...
			release_node_latch(c); // release the LATCH_SH on c
		c = child;
	}
	// c is leaf		
...
RC IndexBTree::insert_into_leaf(glob_param params, bt_node * leaf, idx_key_t key, itemidData * item) {
	UInt32 i, insertion_point;
    insertion_point = 0;
	int idx = leaf_has_key(leaf, key);	
	if (idx >= 0) {
		item->next = (itemidData *)leaf->pointers[idx];
...
RC IndexBTree::split_lf_insert(glob_param params, bt_node * leaf, idx_key_t key, itemidData * item) {
    RC rc;
...
//	printf("%08x\n", id);
	...
	itemidData * temp_pointers[BTREE_ORDER];
    insertion_index = 0;
...
//		new_leaf->pointers[j] = (itemidData *)leaf->pointers[i];
        temp_keys[j] = leaf->keys[i];
        temp_pointers[j] = (itemidData *)leaf->pointers[i];
    }
...
RC IndexBTree::insert_into_parent(
	glob_param params,
	bt_node * left, 
	idx_key_t key, 
	bt_node * right) {
...
RC IndexBTree::insert_into_new_root(
...
int IndexBTree::leaf_has_key(bt_node * leaf, idx_key_t key) {
	for (UInt32 i = 0; i < leaf->num_keys; i++) 
		if (leaf->keys[i] == key)
			return i;
	return -1;
...
UInt32 IndexBTree::cut(UInt32 length) {
	if (length % 2 == 0)
        return length/2;
    else
        return length/2 + 1;
...
/*
void IndexBTree::print_btree(bt_node * start) {
	if (roots == NULL) {
		cout << "NULL" << endl;
		return;
	}
	...
	do {
		...
		while (c != NULL) {
			for (int i = 0; i < c->num_keys; i++) {
				RowData * r = (RowData *)((itemidData*)c->pointers[i])->location;
				if (c->is_leaf)
					printf("%lld(%lld,%d),", 
						c->keys[i], 
						r->get_uint_value(0),
						((itemidData*)c->pointers[i])->valid);
				else 
					printf("%lld,", c->keys[i]);
			}
			cout << "|";
