...
#define M_ASSERT(cond, str) \
	if (!(cond)) {\
		printf("ASSERTION FAILURE [%s : %d] message:%s\n", __FILE__, __LINE__, str);\
    fflush(stdout); \
		exit(0); \
	}
...
#define SET_STATS(tid, name, value) \
	if (STATS_ENABLE && simulate_man->is_warmup_done()) \
...
#define ARR_PTR_MULTI(type, name, size, scale) \
	name = new type * [size]; \
	if (g_alloc_part || THREAD_ALLOC) { \
		for (UInt32 i = 0; i < size; i ++) {\
			UInt32 padsize = sizeof(type) * (scale); \
			if (g_pad_mem && padsize % CL_SIZE != 0) \
				padsize += CL_SIZE - padsize % CL_SIZE; \
			name[i] = (type *) alloc_memory.alloc(padsize); \
...
/*
  ...
  alloc_memory.free(query->part_to_access,sizeof(uint64_t)*query->part_num); \
  alloc_memory.free(query->requests,sizeof(ycsb_query)*query->request_cnt); \
  alloc_memory.free(query,sizeof(ycsb_query)); 
  */
...
public:
	itemidData() { };
	itemidData(Data_type type, void * loc) {
        this->type = type;
        this->location = loc;
    };
	Data_type type;
	...
	bool valid;
	void init();
	bool operator==(const itemidData &other) const;
	bool operator!=(const itemidData &other) const;
	void operator=(const itemidData &other);
};
...
uint64_t acquire_ts(); // return: in ns

class myrand {
public:
	void init(uint64_t rand_seed);
	uint64_t next();
private:
	uint64_t rand_seed;
};
