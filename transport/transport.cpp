...
void Transport::read_ifconfig(const char * ifaddr_file) {
	...
	ifaddr = new char *[g_node_total_cnt];
  ...
  assert(cnt == g_node_total_cnt);
}

uint64_t Transport::get_socket_count() {
  uint64_t sock_cnt = 0;
  if(ISCLIENT)
    sock_cnt = (g_node_total_cnt)*2 + g_cl_send_thd_cnt * g_servers_per_client;
  else
    sock_cnt = (g_node_total_cnt)*2 + g_cl_send_thd_cnt;
  return sock_cnt;
}
...
Socket * Transport::get_socket() {
  ...
  Socket * socket = (Socket*) alloc_memory.align_alloc(sizeof(Socket));
  new(socket) Socket();
...
uint64_t Transport::get_port_id(uint64_t src_node_id, uint64_t dest_node_id) {
  ...
  port_id += g_node_total_cnt * dest_node_id;
  port_id += src_node_id;
...
uint64_t Transport::get_port_id(uint64_t src_node_id, uint64_t dest_node_id, uint64_t send_thread_id) {
  ...
  port_id += src_node_id;
  DEBUG("%ld\n",port_id);
//  uint64_t max_send_thread_cnt = g_send_thd_cnt > g_cl_send_thd_cnt ? g_send_thd_cnt : g_cl_send_thd_cnt;
//  port_id *= max_send_thread_cnt;
  port_id += send_thread_id * g_node_total_cnt * g_node_total_cnt;
  DEBUG("%ld\n",port_id);
  port_id += TPORT_PORT;
  DEBUG("%ld\n",port_id);
...
uint64_t Transport::get_port_id(uint64_t src_node_id, uint64_t dest_node_id, uint64_t send_thread_id) {
  ...
  DEBUG("%ld\n",port_id);
  port_id += send_thread_id * g_node_total_cnt * 2;
  DEBUG("%ld\n",port_id);
  port_id += TPORT_PORT;
  DEBUG("%ld\n",port_id);
  printf("Port ID:  %ld, %ld -> %ld : %ld\n",send_thread_id,src_node_id,dest_node_id,port_id);
...
void Transport::init() {
  ...
  for(uint64_t node_id = 0; node_id < g_node_total_cnt; node_id++) {
    if(node_id == g_node_id)
      continue;
    // Listening ports
    if(ISCLIENTN(node_id)) {
      for(uint64_t client_thread_id = g_cl_thd_cnt + g_cl_rem_thd_cnt; client_thread_id < g_cl_thd_cnt + g_cl_rem_thd_cnt + g_cl_send_thd_cnt; client_thread_id++) {
        uint64_t port_id = get_port_id(node_id,g_node_id,client_thread_id % g_cl_send_thd_cnt);
        Socket * sock = bind(port_id);
        recv_sockets.push_back(sock);
        DEBUG("Socket insert: {%ld}: %ld\n",node_id,(uint64_t)sock);
      }
    } else {
      for(uint64_t server_thread_id = g_thd_cnt + g_rem_thd_cnt; server_thread_id < g_thd_cnt + g_rem_thd_cnt + g_send_thd_cnt; server_thread_id++) {
        uint64_t port_id = get_port_id(node_id,g_node_id,server_thread_id % g_send_thd_cnt);
        Socket * sock = bind(port_id);
        recv_sockets.push_back(sock);
        DEBUG("Socket insert: {%ld}: %ld\n",node_id,(uint64_t)sock);
      }
    ...
    if(ISCLIENTN(g_node_id)) {
      for(uint64_t client_thread_id = g_cl_thd_cnt + g_cl_rem_thd_cnt; client_thread_id < g_cl_thd_cnt + g_cl_rem_thd_cnt + g_cl_send_thd_cnt; client_thread_id++) {
        uint64_t port_id = get_port_id(g_node_id,node_id,client_thread_id % g_cl_send_thd_cnt);
      ...
      for(uint64_t server_thread_id = g_thd_cnt + g_rem_thd_cnt; server_thread_id < g_thd_cnt + g_rem_thd_cnt + g_send_thd_cnt; server_thread_id++) {
        uint64_t port_id = get_port_id(g_node_id,node_id,server_thread_id % g_send_thd_cnt);
...
void Transport::send_msg(uint64_t send_thread_id, uint64_t dest_node_id, void * sbuf,int size) {
  ...
  int rc = -1;
  while(rc < 0 && (!simulate_man->is_setup_done() || (simulate_man->is_setup_done() && !simulate_man->is_done()))) {
    rc= socket->sock.send(&buf,NN_MSG,NN_DONTWAIT);
  }
  //nn_freemsg(sbuf);
  DEBUG("%ld Batch of %d bytes sent to node %ld\n",send_thread_id,size,dest_node_id);

  INC_STATS(send_thread_id,msg_send_time,acquire_ts() - begintime);
  INC_STATS(send_thread_id,msg_send_cnt,1);
}

// Listens to sockets for messages from other nodes
std::vector<Msg*> * Transport::recv_msg(uint64_t thd_id) {
	int bytes = 0;
	void * buf;
  uint64_t begintime = acquire_ts();
  std::vector<Msg*> * msgs = NULL;
  //uint64_t ctr = begintime % recv_sockets.size();
  uint64_t rand = (begintime % recv_sockets.size()) / g_this_rem_thread_cnt;
  ...
  if(ctr >= recv_sockets.size())
    return msgs;
  if(g_this_rem_thread_cnt < g_node_total_cnt) {
  ...
  while(bytes <= 0 && (!simulate_man->is_setup_done() || (simulate_man->is_setup_done() && !simulate_man->is_done()))) {
	...
	if(bytes <= 0 ) {
    INC_STATS(thd_id,msg_recv_idle_time, acquire_ts() - begintime);
    return msgs;
	}

  INC_STATS(thd_id,msg_recv_time, acquire_ts() - begintime);
	INC_STATS(thd_id,msg_recv_cnt,1);

	begintime = acquire_ts();

  msgs = Msg::create_messages((char*)buf);
  DEBUG("Batch of %d bytes recv from node %ld; Time: %f\n",bytes,msgs->front()->return_node_id,simulate_man->seconds_from_begin(acquire_ts()));

	nn::freemsg(buf);	

	INC_STATS(thd_id,msg_unpack_time,acquire_ts()-begintime);
  return msgs;
}
...
uint64_t Transport::simple_recv_msg() {
  ...
  if(g_node_id == 0)
	...
	//printf("%d bytes, %f s\n",bytes,((float)(acquire_ts()-time)) / BILLION);
